...
echo "deb [arch=amd64] https://archive.cloudera.com/cm5/ubuntu/${OS_RELEASE}/amd64/cm/ ${OS_RELEASE}-cm5.12.1 contrib" > /etc/apt/sources.list.d/cloudera-manager.list
curl -L "https://archive.cloudera.com/cm5/ubuntu/${OS_RELEASE}/amd64/cm/archive.key" | apt-key add -

echo "deb [arch=amd64] http://repo.saltstack.com/apt/ubuntu/${OS_VERSION}/amd64/archive/2015.8.11/ ${OS_RELEASE} main" > /etc/apt/sources.list.d/saltstack.list
curl -L "http://repo.saltstack.com/apt/ubuntu/${OS_VERSION}/amd64/archive/2015.8.11/SALTSTACK-GPG-KEY.pub" | apt-key add -
